/*ques>You are given a string s and an integer repeatLimit. Construct a new string repeatLimitedString using 
...
public:
    string repeatLimitedString(string s, int repeatLimit) 
    {
      ...
      while(true)
      {
        ...
        for(;i>=0;i--)
        {
          if(ans.size() && i==ans.back()-'a' && bitMap[i])
          {
            one=true;
            continue;
          }
